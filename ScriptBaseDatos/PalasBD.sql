...
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1 COMMENT='registro de alumno';
...
INSERT INTO `alumno` (`ident`,`usuario`,`carnet`,`estado`,`nombre1`,`nombre2`,`apellido1`,`apellido2`,`genero`,`fnacimiento`,`lnacimiento`,`direccion`,`telefono`,`religion`,`enfermedades`,`medicamentos`,`alergico`,`aprendizaje`,`escuelaprevia`,`grado_aprobado`,`grado_actual`,`correo`) VALUES 
 (1,NULL,'',0,'Iris','Yamileth','Pineda','Ayala','F','0012-07-05 00:00:00','San Esteba Catarina, San Vicente','Col San Antonio, Av. Olimpica #6, C.D.','22163701','SUD','ninguna','ninguno','ninguna','ninguno','ERMUCSA',NULL,6,'irispineda85@hotmail.com'),
 (2,NULL,'',0,'Jose','Eduardo','Lopez','Perez','M','0018-01-04 00:00:00','San Salvador','Colonia La Rabida San Salvador','22163701','Catolico','Ninguna','Ninguno','Ninguno','Ninguno','Centro Escolar Don Luis',NULL,1,'irispineda85@hotmail.com');
...
INSERT INTO `docente` (`ident`,`usuario`,`nombre1`,`nombre2`,`apellido1`,`apellido2`,`direccion`,`telefono`,`fnacimiento`,`fcontrato`) VALUES 
 ...
 (2,NULL,'Flor','Antonia','Sevilla','Alvarado','Col. La Rabida, #1204','2222222','0017-12-06','0017-12-06'),
 (3,NULL,'Adela','Jacinta','Noriega','Vega','Barrio El Calvario #1504, San Salvador','28282828',NULL,NULL),
 (4,NULL,'JOSE','MAURICIO','FLORES','PASAN','CALLE LA REFORMA #1024','22195678','0017-12-01','0017-07-06');
...
CREATE TABLE `evaluacion` (
  ...
  `superiodo` int(10) unsigned NOT NULL COMMENT 'bimestre en el que corresponde la evaluacion',
  ...
  KEY `FK_evaluacion_3` (`superiodo`),
  CONSTRAINT `FK_evaluacion_3` FOREIGN KEY (`superiodo`) REFERENCES `subperiodo` (`ident`),
...
CREATE TABLE `familiar` (
  ...
  `tdoc` varchar(1) DEFAULT NULL COMMENT 'tipo de documento de identificacion DUI (nacionales), Pasaporte',
  `ndoc` varchar(45) DEFAULT NULL COMMENT 'numero del documento de identificacion',
  PRIMARY KEY (`ident`),
...
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=latin1 COMMENT='configuracion de la seccion';
...
CREATE TABLE `materia` (
  ...
  PRIMARY KEY (`ident`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=latin1 COMMENT='configuracion de materias';
...
CREATE TABLE `matricula` (
  ...
  `seccion` int(10) unsigned NOT NULL COMMENT 'grado en el que esta inscrito',
  `estado` varchar(1) DEFAULT NULL COMMENT 'estado de la matricula: A(activa), B(baja), D(desercion), P(aprobado), R(reprobado)',
  `fecha` date DEFAULT NULL COMMENT 'fecha en que se realizo la matricula',
  PRIMARY KEY (`ident`),
  KEY `FK_matricula_1` (`alumno`),
  KEY `FK_matricula_2` (`seccion`) USING BTREE,
  CONSTRAINT `FK_matricula_1` FOREIGN KEY (`alumno`) REFERENCES `alumno` (`ident`),
  CONSTRAINT `FK_matricula_2` FOREIGN KEY (`seccion`) REFERENCES `seccion` (`ident`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1 COMMENT='datos de la matricula del alumno';
...
INSERT INTO `matricula` (`ident`,`alumno`,`seccion`,`estado`,`fecha`) VALUES 
 (1,1,1,'A','2014-11-18');
...
--
-- Definition of table `subperiodo`
--

DROP TABLE IF EXISTS `subperiodo`;
CREATE TABLE `subperiodo` (
  `ident` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `periodo` int(10) unsigned NOT NULL COMMENT 'Periodo',
  `descripcion` varchar(45) NOT NULL COMMENT 'Descripcion, ejemplo Bimestre 1, Trimestre 1',
  `estado` varchar(1) NOT NULL COMMENT 'Estado del subperiodo',
  `inicio` date NOT NULL COMMENT 'Fecha de inicio',
  `fin` date NOT NULL COMMENT 'Fecha de fin',
  PRIMARY KEY (`ident`),
  KEY `FK_subperiodo_1` (`periodo`),
  CONSTRAINT `FK_subperiodo_1` FOREIGN KEY (`periodo`) REFERENCES `periodo` (`ident`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Detalle del periodo';

--
-- Dumping data for table `subperiodo`
--

/*!40000 ALTER TABLE `subperiodo` DISABLE KEYS */;
/*!40000 ALTER TABLE `subperiodo` ENABLE KEYS */;
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1 COMMENT='usuarios del aplicativo';
...
INSERT INTO `usuario` (`ident`,`usuario`,`password`,`rol`,`estado`) VALUES 
 ...
 (3,'alumno@pa12001','ca68b5165c6e02cf',4,'A'),
 (4,'alumno@pa12001','40572e70fc5ac0ae',4,'A');
